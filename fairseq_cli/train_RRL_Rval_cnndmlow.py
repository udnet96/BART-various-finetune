...
from sms import mes, make_logger, mes2
    ...
    if distributed_utils.is_master(cfg.distributed_training):
        ...
        for i, arg in enumerate(sys.argv):
            if str(arg).count('save-dir') > 0:
                ...
                break
        mes2("done training. | {:.2f} val loss | {} epochs | savedir : {} | {:.1f} mins".format(best_val_loss,
                                                                                                int(
...
def summa_gen(cfg: DictConfig, target_src=None, bart=None) -> str:
    ...
    mode = src.split('/')[0].replace('.', '').replace('/', '')
    if mode == 'cnndm':
