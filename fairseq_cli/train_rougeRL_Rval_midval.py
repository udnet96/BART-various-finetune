...
def main(cfg: DictConfig) -> None:
    ...
    """ 
    # for val loss test ..
    # bart model hypo generation method without from_pretrained
    bart = BARTHubInterface(cfg, task, trainer.get_model())  
    beam, lenpen, max_len_b, min_len, data = 6, 1.0, 60, 10, 'xsum'
    bart.cuda()
    bart.eval()
    bart.half()
    with open('xsum/val_30.source') as f, open('xsum/val_30.target') as tgt:
        valsrc = f.readlines()
    ...
    for i in range(len(hypotheses_batch[:5])):
        scores = rouge.get_scores(hypotheses_batch[i], valtgt[i])
        R1_scores.append(scores[0]['rouge-1']['f'])
    ...
    while epoch_itr.next_epoch_idx <= max_epoch:
        ...
        valid_losses, should_stop, best_val_loss, cur_patience = train(cfg, trainer, task, epoch_itr, rouge_val=False,
                                                                       orig_state=orig_state_dict,
                                                                       cur_best_loss=best_val_loss,
                                                                       curr_patience=cur_patience)
        # valid_losses = rouge_validation(cfg, task, trainer, './xsum/val_30')
        """
...
def train(
        cfg: DictConfig, trainer: Trainer, task: tasks.FairseqTask, epoch_itr, rouge_val, orig_state=None,
        cur_best_loss=1e10, curr_patience=0
) -> Tuple[List[Optional[float]], bool, float, int]:
    ...
    for i, samples in enumerate(progress):
        with metrics.aggregate("train_inner"), torch.autograd.profiler.record_function(
            ...
            bart = copy.deepcopy(trainer.get_model())
            log_output = trainer.train_step(samples, rouge_rl=is_rouge_rl, bart=bart)
        ...
        valid_losses, should_stop = validate_and_save(
            ...
            num_samples=len(samples), val_cnt=val_counter, batch_cnt=i, batch_per_epoch=len(progress)
        )

        if valid_losses[0] != None:
            print('$$$$$$ current val counter : {} $$$$$$$'.format(val_counter))
            val_counter += 1
            logger.info('prev val best : {}'.format(best_val_loss))
            if valid_losses[0] < best_val_loss:
                best_val_loss = valid_losses[0]
    ...
    return valid_losses, should_stop, best_val_loss, cur_patience
...
def validate_and_save(
    ...
    # by ud
    val_rate = 0.25
    if val_rate * batch_per_epoch > int(val_rate * batch_per_epoch):
        val_interval = int(val_rate * batch_per_epoch) + 1
    else:
        val_interval = int(val_rate * batch_per_epoch)
    if batch_per_epoch != None:
        if batch_cnt >= val_cnt * val_interval and batch_per_epoch != val_cnt * val_interval:
            do_save, do_validate = True, True

    # Validate
    valid_losses = [None]

    val_file = './xsum/val_low'

    if do_validate:
        if rouge_val:
            valid_losses = [rouge_validation(cfg, task, trainer, val_file, epoch_itr.epoch)]
        else:
