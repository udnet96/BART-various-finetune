...
def main(cfg: DictConfig) -> None:
    ...
    while epoch_itr.next_epoch_idx <= max_epoch:
        ...
        valid_losses, should_stop = train(cfg, trainer, task, epoch_itr,
                                          rouge_val=False, orig_state=orig_state_dict)
...
def train(
    ...
    for i, samples in enumerate(progress):
        ...
        valid_losses, should_stop = validate_and_save(
            ...
            num_samples=len(samples), val_cnt=val_counter, batch_cnt=i
        )
...
def validate_and_save(
    ...
    # by ud, mid-epoch validation
    """
    val_rate = 0.25
    val_interval = int(val_rate * 200)
    if num_samples != None:
        num_trained = (batch_cnt * num_samples)
        if num_trained >= val_cnt * val_interval:
