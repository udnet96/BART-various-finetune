...
@register_criterion("label_smoothed_cross_entropy_cossim")
class LMCE_cossim_Criterion(FairseqCriterion):
    def __init__(
    ...
    def forward(self, model, sample, reduce=True, wolog=False,
                emb_model=None, is_cossim=False, bpe=None, tgt_dict=None, cnt=0):
        ...
        # if is_cossim:
        if is_cossim and model.training:
            # SBERTmodel = SentenceTransformer('stsb-distilbert-base')
            hypo_tok = torch.argmax(utils.log_softmax(input_logits[0], dim=-1), -1)  # maxpool
            # print('sftmax, argmax hypo tok : {} || {}'.format(utils.log_softmax(input_logits[0], dim=-1).shape,
            #                                                  hypo_tok.shape))
            hypo_txt = bpe.decode(tgt_dict.string(hypo_tok)).replace('<pad>', '')
            gold_txt = bpe.decode(tgt_dict.string(sample["target"])).replace('<pad>', '')  # gold
            hypo, gold = torch.tensor(emb_model.encode(hypo_txt, show_progress_bar=False)), \
                         torch.tensor(emb_model.encode(gold_txt, show_progress_bar=False))
            # print('hypo, gold : {}, {}'.format(hypo.unsqueeze(0).shape, gold.unsqueeze(0).shape))
            cossim = torch.cosine_similarity(hypo.unsqueeze(0), gold.unsqueeze(0))
            # print('ip logits : ', input_logits[0].shape)
            alpha = 0.3
            if cnt % 200 == 0:
                print('__*__' * 20)
                print('hypo : {}'.format(hypo_txt))
                print('gold : {}'.format(gold_txt))
                print('cossim : {}'.format(loss * alpha * cossim[0]))
                print('loss, cossim, alpha : {}, {}, {}'.format(orig_loss.data,
                                                                cossim.data * alpha, alpha))
                print('__*__' * 20)
            loss -= loss * alpha * cossim[0]
        ...
        if is_cossim:
            logging_output.update(
                symm_kl=utils.item(cossim.data) if reduce else cossim.data
            )
        """
        if model.training:
            logging_output.update(
