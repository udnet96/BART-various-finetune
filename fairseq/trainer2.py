...
class Trainer(object):
    ...
    def valid_step(self, sample, raise_oom=False, prev_state=None):
        ...
        with torch.no_grad():
            ...
            try:
                ...
                for key in state_dict:
                    if torch.equal(state_dict[key], prev_state[key]):
                        coun += 1
                    else:
                        logger.info('prev state : {}'.format(prev_state[key]))
                        logger.info('curr state : {}'.format(state_dict[key]))
                logger.info('coun : {}, len : {}'.format(coun, len(state_dict.items())))

                cnt = 0
                for key in state_dict:
                    logger.info(
                        'loaded .. key : {}, {}\nvalue : {}'.format(key, state_dict[key].shape,
                                                                    state_dict[key]))
                    cnt += 1
                    if cnt > 2:
                        break
