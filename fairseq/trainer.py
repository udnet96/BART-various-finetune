...
class Trainer(object):
    ...
    def train_step(self, samples, raise_oom=False, rouge_rl=False, bart=None, wolog=False,
        ...
        for i, sample in enumerate(samples):
            ...
            try:
                with maybe_no_sync():
                    ...
                    if rouge_rl:
                        loss, sample_size_i, logging_output = self.task.train_step_withRL(
                            cfg=self.cfg,
                            task=self.task,
                            sample=sample,
                            model=self.model,
                        ...
                        loss, sample_size_i, logging_output = self.task.train_step(
                            ...
                            update_num=self.get_num_updates(),
                            ignore_grad=is_dummy_batch,
                            wolog=wolog, emb_model=emb_model, is_cossim=is_cossim, bpe=bpe
                        )
                    else:
                        loss, sample_size_i, logging_output = self.task.train_step(
                            sample=sample,
