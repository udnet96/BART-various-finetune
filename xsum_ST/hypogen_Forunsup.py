...
def hypogen(ckpt, iter=0, is_test=False, num_to_write=-1, cal_rouge=True, summa_gen=True):
    ...
    """
    if num_to_write > 0:
        src_for_bin='./xsum_ST/unsupdata_forbin.source'
        with open(src_for_bin, 'w') as fout, open(src_file) as fin:
                src_lines = fin.readlines()
                for line in src_lines[:n_to_write]:
                    fout.write(line)
    """
    ...
    if summa_gen:
        bart = BARTModel.from_pretrained(
            './', checkpoint_file=ckpt, data_name_or_path='xsum-bin_low_200_100'
        )
        bart.cuda()
        bart.eval()
        bart.half()
    ...
    # mes('%s summa gened.' % server + filename.split('/')[-1] + 'fintime : ' + str(fintime))
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 4:
        ...
        os.system('cp {} {}'.format(hypofile, './xsum_ST/unsupdata_forbin.target'))
    elif len(sys.argv) >= 3:
